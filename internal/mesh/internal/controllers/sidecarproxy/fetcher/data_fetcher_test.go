...
type dataFetcherSuite struct {
	...
	webComputedDestinationsData *pbmesh.ComputedDestinations
...
func (suite *dataFetcherSuite) SetupTest() {
	...
	suite.webComputedDestinationsData = &pbmesh.ComputedDestinations{
...
func (suite *dataFetcherSuite) TestFetcher_FetchExplicitDestinationsData() {
	...
	testutil.RunStep(suite.T(), "computed destinations not found", func(t *testing.T) {
		...
		compDest := resourcetest.Resource(pbmesh.ComputedDestinationsType, suite.webProxy.Id.Name).
			...
			WithTenancy(resource.DefaultNamespacedTenancy()).
			Build()
		c.TrackComputedDestinations(resourcetest.MustDecode[*pbmesh.ComputedDestinations](t, compDest))
